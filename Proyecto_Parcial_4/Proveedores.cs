...
namespace Proyecto_Parcial_4
{
    public partial class FMRProveedores : Form
    {
        ...
        {
            DialogResult decision = MessageBox.Show("¿Seguro que desea actualizar el inventario del artículo?", "ADVERTENCIA", MessageBoxButtons.YesNo, MessageBoxIcon.Stop);
            if (decision == DialogResult.Yes)
            {
                ...
                bool correcto = false;
                //Se verifica que los valores que deben ser numerico si sean números, de lo contrario, darle posibilidad
                //al usuario de volver a ingresa un valor
                while (correcto == false)
                {
                    try
                    {
                        double suma = double.Parse(TBUnidades.Text) + 2;
                        correcto = true;
                    }
                    catch
                    {
                        TBUnidades.Text = Interaction.InputBox("El valor ingresado en la cantidad de unidades devueltas no es un número, por favor vuelva a ingresarlo", "ATENCIÓN");
                        correcto = false;
                    }
                }
                for (int row = 0; row < tabla.Rows.Count; row++)
        ...
        {
            DialogResult decision = MessageBox.Show("¿Seguro que desea agregar este artículo?", "ADVERTENCIA", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (decision == DialogResult.Yes)
            {
                ...
                while (correcto == false)
                {
                    try
                    {
                        int suma = int.Parse(TBCantidad.Text) + 2;
                        correcto = true;
                    }
                    catch
                    {
