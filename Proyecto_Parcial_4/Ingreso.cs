...
{
    public partial class FMRIngreso : MaterialSkin.Controls.MaterialForm
    {
        public FMRIngreso()
        {
            InitializeComponent();
        ...
        {
            //Se emplea un booleano para verificar la conexion con el servidor
            bool server = false;
            try
            {
                SqlConnection prueba = new SqlConnection(string.Format("Data Source = {0};Initial Catalog = Tienda;Integrated Security = True", CBServer.Text));
                prueba.Open();
                server = true;
            }
            catch
            {
                //Se envia un mensaje de aviso al usuario, que comunique el fallo en el server
                MessageBox.Show("Error en la conexión con el servidor, por favor verifique","ERROR SERVER");
                server = false;
                TBUser.Clear();
                TBPassword.Clear();
            }
            if(server == true)
            {
                ...
                SqlConnection conexion = new SqlConnection(string.Format("Data Source = {0};Initial Catalog = Tienda;Integrated Security = True", CBServer.Text));
                conexion.Open();
                ...
                else
                {
                    ...
                    //de quien ingresa
                    FRMVenta formulario = new FRMVenta(administrador, CBServer.Text);
                    formulario.Show();
